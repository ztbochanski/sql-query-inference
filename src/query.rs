...
impl Query {
    ...
    fn extract_columns(&self) -> Vec<String> {
        ...
        for field in [&self.select_columns, &self.join_columns, &self.where_columns, &self.agg_columns].iter() {
    ...
    pub fn map_tables_columns(&self) -> Vec<TableMetadata> {
        ...
        let table_names: HashMap<_, _> = tables
            ...
            .map(|t| {
                ...
                if let Some(table_with_schema) = parts.get(0) {
                    let table_with_schema = table_with_schema.trim_matches(|c| c == '[' || c == '\"');
                    let table_name = table_with_schema.split('.').last().unwrap().trim_matches(|c| c == '\"');
                    let alias = parts.get(1).map_or("", |a| a.trim_matches(|c| c == '\"' || c == ']'));
                    (table_name, alias)
                } else {
                    ("", "")
                }
        ...
        if columns.is_empty() {
            ...
            let table_info: Vec<TableMetadata> = vec![TableMetadata { table_name: table_name_tokens, columns: column_name_tokens }];
            return table_info;
        }
        ...
        for column in columns {
            ...
            let table = table_names.keys().find(|&k| table_names[k] == table_name).unwrap_or(&table_name);

            table_columns_map.entry(table.to_string()).or_insert(Vec::new()).push(column_name.to_string()); 
        }
        
        let table_info: Vec<TableMetadata> = table_columns_map.iter().map(|(k, v)| TableMetadata { table_name: k.to_string(), columns: v.clone()}).collect();
        table_info
    }
}
